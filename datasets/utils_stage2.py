...
def voxelize(
    ...
    for sample in batch:
        ...
        pre_generated_masks = np.load(f'{generate_masks_path}/{mode}/masks/' + sample[3] + '.npy')
        ...
        mask_score = np.load(f'{generate_masks_path}/{mode}/scores/' + sample[3] + '.npy')
...
def get_instance_masks(
    ...
    for batch_id in range(len(generated_masks)):
        ...
        masks = []
        segment_masks = []
        select_indices = np.where(generated_masks_scores[batch_id] > use_masks_score)[0]
        new_select_indices = select_indices[select_indices < use_masks_th]

        for instance_id in new_select_indices:

            label_id = list_labels[batch_id][0, 0]
            label_ids.append(label_id)
            masks.append(generated_masks[batch_id][:,instance_id])
